...
func getIconHandler(c echo.Context) error {
	...
	var iconHash string
	if err := dbConn.GetContext(ctx, &iconHash, "SELECT hash FROM icons WHERE user_id = ?", user.ID); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
	...
	if match := c.Request().Header.Get("If-None-Match"); match != "" && match == iconHash {
...
func fillUserResponse(ctx context.Context, userModel UserModel) (User, error) {
	...
	var iconHash string
	if err := dbConn.GetContext(ctx, &iconHash, "SELECT hash FROM icons WHERE user_id = ?", userModel.ID); err != nil {
		if !errors.Is(err, sql.ErrNoRows) {
			return User{}, err
		}
	}
